...
module.exports.updateVulnerability = async (req, res) => {
    try {
        let { vulnerabilityIdentifier = undefined, severity = undefined, title = undefined, category = undefined, summary = undefined, impact = undefined, remediation = undefined } = req.body;
        ...
        const updateObject = Object.assign(
            {},
            ...Object.entries({ vulnerabilityIdentifier, severity, title, category, summary, impact, remediation })
                .filter(([key, value]) => value !== undefined)
                .map(([key, value]) => ({ [key]: value }))
        );

        if (Object.keys(updateObject).length > 0) {
            const vulnerability = await Vulnerability.findByIdAndUpdate(req.params.vulnerabilityId, updateObject, { new: true });
            createLog("info", `The vulnerability "${vulnerabilityIdentifier}" was updated successfully`)
